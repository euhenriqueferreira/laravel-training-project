...
<button type="{{ $type}}" {{ $attributes }} @class([
    ...
    "text-sm text-center px-4 py-2 rounded-md  border-2 text-lg font-semibold flex items-center justify-center text-gray-800 dark:text-gray-800 border-gray-800 hover:scale-[1.03] transition" => $outline
])>
    {{ $slot }}
</button>